...
export default function Modal({ open, setOpen, profile}) {
  const {change_password, activeForm} = useContext(AuthContext);
  ...
  const handleSave = async () => {
    await change_password(password, password2, handleNotify);
    localStorage.setItem('activeForm', activeForm);
    window.location.reload();
    handleNotify("Changes applied.", "Your password is updated", "success")
    setOpen(false);
                      ...
                      <div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                          <div className="sm:col-span-3">
                              <label htmlFor="new-password"
                                     className="block text-sm font-medium leading-6 text-gray-900">New
                                  password</label>
                              <div className="mt-2">
                                  <input
                                      id="new-password"
                          ...
                          <div className="sm:col-span-3">
                              <label htmlFor="last-name" className="block text-sm font-medium leading-6 text-gray-900">Confirm
                                  your
                                  new password</label>
                              <div className="mt-2">
                                  <input
